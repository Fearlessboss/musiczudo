...
========================================================
   ...
   - Bullet-proof: DB fail ho to bhi bot chalta rahega
...
#                     DATABASE
# ============================================================
# Mongo client with proper Atlas-friendly timeouts + TLS.
# We DO NOT crash if Mongo is unreachable — we retry forever in background.
mongo = MongoClient(
    MONGO_URL,
...
# In-memory fallback cache (used if DB is temporarily down)
_DB_READY = False
_MEM_SETTINGS: dict = {}
_MEM_BUTTONS_DEFAULT = [{
    "_id": "default0",
...
def _db_init_sync():
    # ping first to detect real connectivity
...
async def db_init_with_retry():
    """Keep retrying DB init in background — never crash the bot."""
    global _DB_READY
    attempt = 0
    while not _DB_READY:
        attempt += 1
        try:
            ...
            _DB_READY = True
            log.info("✅ MongoDB connected & initialised (attempt %d)", attempt)
            return
        except (ServerSelectionTimeoutError, PyMongoError) as e:
            wait = min(60, 5 * attempt)
            log.warning(
                "⚠️ Mongo not ready (attempt %d): %s — retrying in %ds. "
                "Bot will keep running with in-memory fallback.",
                attempt, str(e).splitlines()[0], wait,
            )
            await asyncio.sleep(wait)
        except Exception as e:
            wait = min(60, 5 * attempt)
            log.warning("⚠️ Unexpected DB init error: %s — retry in %ds", e, wait)
            await asyncio.sleep(wait)
...
async def get_setting(key: str, default=None):
    if not _DB_READY:
        return _MEM_SETTINGS.get(key, default)
    try:
        doc = await asyncio.to_thread(settings_col.find_one, {"_id": key})
        return doc["value"] if doc else default
    except Exception as e:
        log.warning("get_setting fallback (%s): %s", key, e)
        return _MEM_SETTINGS.get(key, default)


async def set_setting(key: str, value):
    _MEM_SETTINGS[key] = value  # always cache locally
    if not _DB_READY:
        return
    try:
        ...
        log.warning("set_setting failed (%s): %s", key, e)
...
async def get_all_user_ids():
    ...
    try:
        ...
        log.warning("get_all_user_ids failed: %s", e)
        return []
...
async def get_buttons():
    if not _DB_READY:
        return _MEM_BUTTONS_DEFAULT
    try:
        data = await asyncio.to_thread(_get_buttons_sync)
        return data if data else _MEM_BUTTONS_DEFAULT
    except Exception as e:
        log.warning("get_buttons fallback: %s", e)
        return _MEM_BUTTONS_DEFAULT
...
async def count_buttons():
    if not _DB_READY:
        return len(_MEM_BUTTONS_DEFAULT)
    try:
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await guard(update):
        return
    db_status = "🟢 Online" if _DB_READY else "🟠 Reconnecting..."
    text = (
        ...
        "📣 /broadcast\n"
        "   ↳ Send any message to <b>every</b> user.\n\n"
        "📊 /stats\n"
...
async def cmd_addbuttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not _DB_READY:
        await update.message.reply_text(
            "⚠️ Database abhi reconnect ho raha hai. Thodi der baad try karo."
        )
        return ConversationHandler.END
    await update.message.reply_text(
...
async def cmd_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not _DB_READY:
        await update.message.reply_text(
            "⚠️ Database abhi reconnect ho raha hai. Broadcast ke liye DB chahiye."
        )
        return ConversationHandler.END
    await update.message.reply_text(
...
def main():
    ...
    async def _post_init(application: Application):
        # 🔑 DB init ko BACKGROUND me bhejo — bot start hone se na roke
        asyncio.create_task(db_init_with_retry())
        log.info("Bot is up and running 🚀 (DB connecting in background)")
